    ...
    static func scanQRCode() async throws -> String {
        ...
        return await withCheckedContinuation { continuation in
            ...
            let handler: SPQRCodeCallback = { data, vc in
                switch data {
                case .flowWallet(let address), .ethWallet(let address):
...
extension TurboModuleSwift {
    ...
    private static func getAllProfiles() async throws -> [RNBridge.WalletProfile] {
        ...
        for profile in allProfiles {
            ...
            let eoaList = await WalletManager.shared.allEOAAccounts(with: walletEntity, for: profile.uid)
            if !eoaList.isEmpty, let address = accountList.first?.hexAddr {
                let result = eoaList.compactMap {
                    $0.toWalletAccount(parentAddress: address, userId: profile.uid).toRNBridge()
                }
    ...
    static func ethSign(_ hexData: String) -> String? {
        ...
        return try? keyProvider.ethSign(digest: Data(hexData.hexValue), index: index).hexString
    }
