...
final class ProfileManager: ObservableObject {
    ...
    func getAccount(byTxId txId: String) async -> FlowWalletKit.Account? {
        guard !txId.isEmpty else {
            log.warning("[Profile] Empty txId when fetching account")
            return nil
        }

        guard let wallet = await WalletManager.shared.walletEntity else {
            log.warning("[Profile] Wallet entity not ready when fetching account by txId")
            return nil
        }

        do {
            let account = try await wallet.fetchAccountsByCreationTxId(
                txId: Flow.ID(hex: txId.removePrefix("0x")),
                network: currentNetwork
            )
            
            return account
        } catch {
            log.error("[Profile] Fetch account by txId failed: \(txId), error: \(error)")
            return nil
        }
    }

    /// Fetch account by txId and add it to target profile
    /// - Parameters:
    ///   - txId: Transaction id used to create account
    ///   - profile: Target profile that should receive the new account
    func addAccount(byTxId txId: String, to profile: ProfileModel) async {
        guard let fetchedAccount = await getAccount(byTxId: txId) else {
            return
        }

        let latestProfile = loadProfile(userId: profile.uid) ?? profile
        var updatedProfile = latestProfile
        var didUpdate = false

        do {
            let group = try await fetchedAccount.buildWalletAccount(userId: latestProfile.uid, forceZero: true)
            let existingAddresses = Set(
                updatedProfile.accounts
                    .flatMap { $0 }
                    .map { $0.address.lowercased() }
            )
            let newAccounts = group.filter { !existingAddresses.contains($0.address.lowercased()) }

            if !newAccounts.isEmpty {
                var groups = updatedProfile.accounts
                groups.append(newAccounts)
                updatedProfile = updatedProfile.updatingAccounts(to: groups)
                didUpdate = true
            }
        } catch {
            log.error("[Profile] Build wallet account group failed for txId \(txId): \(error)")
        }

        guard didUpdate else {
            log.info("[Profile] No new account to add for txId \(txId)")
            return
        }
        let resultProfile = updatedProfile
        await MainActor.run {
            saveProfile(resultProfile)
        }
    }
