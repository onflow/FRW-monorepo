...
export const useStorageWarning = () => {
  ...
  const cadenceService = useCadence();

  const checkStorageConditions = useCallback(
    async (account: WalletAccount | null) => {
      if (!account?.address) {
      ...
      try {
        // cadenceService is now available from the hook scope
