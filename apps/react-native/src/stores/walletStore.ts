...
export const useWalletStore = create<WalletStore>((set, get) => ({
  ...
  loadAccountsFromBridge: async () => {
    ...
    try {
      ...
      console.log('[WalletStore] Loaded', accounts.length, 'accounts from bridge', accounts);
    } catch (error) {
      console.error('[WalletStore] Failed to load accounts:', error);
      set({
        accounts: [],
        activeAccount: null,
        isLoading: false,
...
export const walletHelpers = {
  ...
  initializeWallet: async () => {
    ...
    console.log('[WalletHelpers] Initialized', accounts.length, 'accounts');
    return accounts;
  },
