...
import { SendPayload } from '@/network/cadence';
import { isFlowToken } from '@/network/cadence/send/utils';
import { FlowService } from '@/service';
import { addressType } from '@/types';
import { NFTModel } from '@/types/NFTModel';
import { TokenInfo } from '@/types/TokenInfo';
import { create } from 'zustand';
import { BalanceData, SendFormData, SendState, TransactionType, WalletAccount } from './types';
...
export const useSendStore = create<SendState>((set, get) => ({
  ...
  createSendPayload: async (): Promise<SendPayload | null> => {
    ...
    try {
      ...
      const { accounts } = await NativeFRWBridge.getWalletAccounts();
      ...
      const payload: SendPayload = {
        ...
        ids: isNFTTransaction
          ? (selectedNFTs
              .map(nft => (nft.id != null ? parseInt(nft.id as string) : undefined))
