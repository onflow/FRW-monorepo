...
export class RecentRecipientsService {
   ...
   */
  static async getAllRecentRecipients(): Promise<WalletAccount[]> {
    try {
   ...
   * Get local recent recipients from MMKV
   */
  static getLocalRecentRecipients(): RecentRecipient[] {
    try {
      const data = storage.getString(RECENT_RECIPIENTS_KEY);
      if (!data) return [];
   ...
   * Get server recent recipients from bridge
   */
  static async getServerRecentRecipients(): Promise<RecentRecipient[]> {
    try {
      const recentData = await NativeFRWBridge.getRecentContacts();
   ...
   */
  static addRecentRecipient(recipient: {
    id?: string;
    name: string;
    address: string;
    emoji?: string;
    ...
    try {
      ...
      // Save to MMKV
      storage.set(RECENT_RECIPIENTS_KEY, JSON.stringify(updated));
   ...
   */
  static clearLocalRecentRecipients(): void {
    try {
      storage.delete(RECENT_RECIPIENTS_KEY);
   ...
   */
  private static mergeAndDeduplicateRecents(
   ...
   */
  static isAddressInRecents(address: string): boolean {
    const localRecents = this.getLocalRecentRecipients();
   ...
   */
  static getRecentRecipientByAddress(address: string): RecentRecipient | null {
    const localRecents = this.getLocalRecentRecipients();
