...
const MultiBackupConnect: React.FC<MultiBackupConnectProps> = ({
  ...
  const getMultiBackup = async () => {
    ...
    try {
      ...
      if (hasDropbox) {
        setShowError(true);
        setErrorMessage('Dropbox multi-backup is not available on V2 workflow yet.');
        return;
      }

      logger.info('[BackupRoute:UI] Multi backup connect -> dedicated multi-backup folder route', {
        sources,
      });
