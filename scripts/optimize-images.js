...
async function optimizeWithImageOptimCLI(filePath, ignoreErrors) {
  ...
  try {
    ...
    if (ignoreErrors) {
      console.warn(`Ignoring ImageOptim failure for ${filePath}`);
      return true;
    }
    return false;
  }
...
async function fallbackOptimization(filePath, ignoreErrors) {
  ...
  try {
    ...
    if (ignoreErrors) {
      console.warn(`Ignoring validation failure for ${filePath}`);
      return true;
    }
    return false;
  }
...
async function optimizeImage(filePath, ignoreErrors) {
  try {
    const ext = path.extname(filePath).toLowerCase();
    ...
    if (os.platform() === 'darwin' && (await checkImageOptimCLI())) {
      return await optimizeWithImageOptimCLI(filePath, ignoreErrors);
    } else {
      // Fallback for non-macOS platforms or when ImageOptim-CLI is not available
      return await fallbackOptimization(filePath, ignoreErrors);
    }
  } catch (error) {
    console.error(`Error processing ${filePath}:`, error.message);
    if (ignoreErrors) {
      console.warn(`Ignoring processing failure for ${filePath}`);
      return true;
    }
    return false;
  }
...
async function main() {
  ...
  const ignoreErrors =
    rawArgs.includes('--ignore-errors') || process.env.IMAGEOPTIM_IGNORE_ERRORS === '1';
  const files = rawArgs.filter((arg) => arg !== '--ignore-errors');

  if (files.length === 0) {
    console.log('Usage: node optimize-images.js <file1> <file2> ... [--ignore-errors]');
    process.exit(1);
  }
  ...
  for (const file of files) {
    const result = await optimizeImage(file, ignoreErrors);
    if (!result) {
      success = false;
    }
  }
