...
class AddressBookPersonPresenter(
    ...
    private val isSendTransactionPage by lazy { BaseActivity.getCurrentActivity()?.javaClass == ReactNativeActivity::class.java }
    ...
    override fun bind(model: AddressBookPersonModel) {
        ...
        with(binding) {
            ...
            } else if (address == WalletManager.getFlowWalletAddress() || EVMWalletManager
                    .isEVMWalletAddress(address)
            ) {
                val emojiInfo =
                    AccountEmojiManager.getEmojiByAddress(WalletManager.getFlowWalletAddress())
