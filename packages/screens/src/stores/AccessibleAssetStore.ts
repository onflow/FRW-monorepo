  ...
  create<AccessibleAssetStore>()(
    subscribeWithSelector((set, get) => ({
      ...
      fetchChildAccountAllowTypes: async (
        ...
        try {
          // Use provided cadenceService or create a new one
          const service = cadenceService || new CadenceService();

          // Fetch the accessible IDs from cache
          const accessibleIds = await service.getChildAccountAllowTypes(
            parentAddress,
            childAddress
          );
