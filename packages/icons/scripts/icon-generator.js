...
// Detect whether an SVG is multicolor (uses multiple fills/strokes or gradients)
function isMulticolorSvg(svgPath) {
  try {
    const raw = fs.readFileSync(svgPath, 'utf8');
    // Gradients, patterns, or url(#...) refs imply multicolor/special effects
    if (/<linearGradient|<radialGradient|stop-color=|url\(#/i.test(raw)) return true;

    // Collect distinct explicit colors from fill/stroke declarations
    const colorRegex = /(fill|stroke)=["']([^"']+)["']/gi;
    const colors = new Set();
    let m;
    while ((m = colorRegex.exec(raw))) {
      const val = (m[2] || '').trim().toLowerCase();
      if (!val) continue;
      if (val === 'none' || val === 'currentcolor' || val.startsWith('url(')) continue;
      colors.add(val);
    }
    return colors.size >= 2; // two or more distinct colors
  } catch {
    return false;
  }
}
...
function generateReactNativeIcon(componentName, svgPath, outputDir) {
  ...
  try {
    ...
    // Use local svgr CLI (avoid npx/network and ensure workspace binary)
    const svgrBin = path.join(__dirname, '..', 'node_modules', '.bin', 'svgr');
    execSync(`"${svgrBin}" --out-dir "${outputDir}" "${svgPath}" --typescript --native --icon`, {
      stdio: 'pipe',
    });
    ...
    if (fs.existsSync(svgrOutputPath)) {
      ...
      const preserveColors = isMulticolorSvg(svgPath);
      ...
      content =
        content
          ...
          .replace(
            ...
            `const ${componentName} = ({ color = "#000", size = 24, width, height, ...props }: SvgProps & { size?: number }) => (`
          )
        ...
        // Color processing applied only for single-color icons
        (preserveColors ? '' : '')
          // Update export statement to match svgr CLI format
          .replace(/export default Svg\w+;/, `export default ${componentName}`)
          .replace(/export default\s+\w+\s*;/, `export default ${componentName};`);

      // As a final safeguard, ensure the export line is correct
